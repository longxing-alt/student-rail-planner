...
const RAIL_LINES = [
  ['哈尔滨', '长春', '沈阳', '北京'],                         // 京哈
  ['北京', '保定', '石家庄', '邢台', '邯郸', '安阳', '新乡', '郑州', '许昌', '漯河', '驻马店', '信阳', '孝感', '武汉', '岳阳', '长沙', '衡阳', '郴州', '韶关', '广州'], // 京广正线(补保定/邢台/邯郸/安阳/新乡/许昌/漯河/驻马店/孝感等, 原简化路径漏站致邯郸等判为区间外)
  ['石家庄', '太原'], // 石太(原并入京广简化路径, 拆出)
  ['郑州', '太原'],   // 郑太(同上)
...
function renderResult(){
  ...
  let html='<div class="card" id="resCard">'+
    ...
    '<div class="sum">'+
    (cc.okAll?(cc.segs.some(s=>s.lowConf)?'联程全程可出 · ⚠含未实测近邻中转':'联程全程可出'):'<b>'+okN+'/'+N+'</b> 段可出 · '+(N-okN)+' 段需另购')+
    hubLineHtml+
  ...
  for(let i=0;i<pts.length;i++){
    if(i>0){
      ...
      if(sgA.hub&&!sgA.inInt){
        html+='<span class="chip warn">'+esc(sgA.hub)+'<span style="display:block;font-size:9px;color:#d97706;font-weight:700;line-height:1.15">'+(sgA.lowConf?'⚠未实测':'中转站')+'</span></span>';
      }
    }
  ...
  if(!cc.okAll){
    html+='<div style="margin-bottom:8px">'+cc.segs.map((sg,i)=>{
      ...
      (sg.inInt?'<span class="chip ok" style="margin-left:8px">✓ 区间内·直达</span>':sg.hub?(sg.lowConf?'<span class="chip warn" style="margin-left:8px">⚠ 近邻中转·未实测</span>':'<span class="chip ok" style="margin-left:8px">✓ 可出·中转</span>'):'<span class="chip no" style="margin-left:8px">✕ 区间外·需购成人票</span>')+hubLine+'</div>'+
      (sg.inInt?'':sg.hub?'<span class="hub-badge" data-tfm="'+i+'">'+(sg.lowConf?'⚠ 近邻中转·可能被拦·推荐':'⇄ 可中转哪里出票·推荐')+esc(sg.hub)+'</span>':'')+
      '</div>';
    }).join('')+'</div>';
...
function chainBestLocal(){
  ...
  for(const s of STATIONS){
    ...
    if(s[1]===S.city)continue; // 家不能与学校同城(北京⇄北京 无法认定); 推荐跨城端点
