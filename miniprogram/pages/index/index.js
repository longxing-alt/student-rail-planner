...
Page({
  ...
  buildSharePath() {
    const S = state.school, H = state.home || state.depart;
  ...
  /* 码图预载: createImage 直接用包内路径(不经临时文件换链(旧实现缓存的临时路径过期会让码图消失)
  ...
  async nextStart() {
    ...
    this.setData({
      departInput: state.depart.name, startName: state.depart.name,
