...
function bandOK(S, H, P) {
  const L = dist(S, H);
  // 真·同城圈(L<110): 端点近邻≤600km 放行 —— 实测依据 广州⇄佛山25km(深圳/珠海/中山/东莞/厦门515km 全放行)、
  // 苏州⇄上海80km(合肥322/徐州458/武汉596 放行, 北京1028km 拦截)。
  // 阈值由 150 收紧至 110: 实测 南京⇄合肥142km 下 开封(440km)/长沙(575km) 被12306拒绝,
  // 说明 142km 已不属同城圈, 不应享受该宽松; 110 卡在 苏沪80 与 宁合142 之间。
  if (L < 110) return dist(S, P) <= 600 || dist(H, P) <= 600;
  const { t, p } = corridor(S, H, P);
